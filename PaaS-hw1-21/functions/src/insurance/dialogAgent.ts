...
export const dialogAgent = async (request:any, dialogMessage: DialogMessage|any): Promise<any> => {
...
export const publish = async (dialogMessage: DialogMessage) => {
   if (dialogMessage.channel && dialogMessage.userId && dialogMessage.replyMessages ) {
       ...
       await googlePubsub.topic(topicName).publisher().publish(dataBuffer).then((res:any) => {
