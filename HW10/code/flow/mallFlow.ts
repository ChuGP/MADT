...
import { mallServiceUrl, shop12ServiceUrl, shop14ServiceUrl } from "./mallFlowConfig"
...
export const bindCustomer = functions.https.onRequest(async (req, res) => {
    corsHandler(req, res, async () => {
        ...
        if (member) {
            ...
            let customer: Customer = await axios.get(mallServiceUrl + `service/${path}/customers?key=memberId&value=${member.memberId}`).then(result => {
...
export const banking = functions.https.onRequest((req, res) => {
    corsHandler(req, res, async () => {
        const bankingChatModel = req.body.bankingChatModel as BankingChatModel
        let serviceUrl
        switch (bankingChatModel.shopId) {
            case "12":
                serviceUrl = shop12ServiceUrl
                break
            case "14":
                serviceUrl = shop14ServiceUrl
                break
        }
        let customer: Customer = await axios.get(serviceUrl + `customers?key=lineId&value=${bankingChatModel.chatId}`).then(result => {
            return result.data.customers[0] as Customer
        })
        const time = new Date().getTime();
        if (customer) {
            ...
            axios.put(serviceUrl + `customers`, { customer })
...
export const carry = functions.https.onRequest((req, res) => {
    corsHandler(req, res, async () => {
        ...
        let customer = await axios.get(serviceUrl + `customers?key=${key}&value=${carryChatModel.chatId}`).then(result => {
