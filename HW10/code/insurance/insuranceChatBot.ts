...
import { DIALOGFLOW, shop, paymentShopId, pubsubConfig, APIGEE ,insuranceSubName, mallFlowUrl , mallServiceUrl } from './insuranceConfig'
...
export const actionDispatcher = async (queryResult, dialogMessage: DialogMessage) => {
...
const errorHandle = (dialogMessage: DialogMessage, error: any, path: string) => {
    ...
    let replyMessages = [{ type: "text", message: errorMessageToCustomer }]
    const replyMessage: ReplyMessage = {
        channel: dialogMessage.channel,
        chatId: dialogMessage.chatId,
